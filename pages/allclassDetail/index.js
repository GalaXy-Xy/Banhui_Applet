...
Page({
  ...
  data: {
      ...
      Closed:true,
      flag:true,
      classTitle:'春风那个吹，战鼓那个擂，这里可以多些点，不限制字数，只要你能写，反正班级首页放不下可以省略，点击查看详情。' , 
      flag_n:true,
      // 主键id
      userID:'',
      // 班级id
  ...
  classList(e){
    var that = this
    var classId_all = that.data.classId
    var userId = that.data.userID
    console.log(e)
    wx.navigateTo({
      url: '/pages/classList/index',
      success(res){
        res.eventChannel.emit('data', {userId,classId_all})
      }
    })
  },
  hide:function(){
  ...
  onLoad(options){
    ...
    wx.request({
      ...
      success(res){
        ...
        that.setData({
          ...
          classTitle:res.data.data.class_desc
        })
        // 判断是否为班长
        wx.request({
          url: API +'api/sel_if_monitor',
          data:{
            classmate_classid:options.calssid,
            classmate_userid:that.data.userID
          },
          success(res){
            console.log(res)
    ...
    wx.request({
      ...
      success(res){
        ...
        // if(res.data.data.classmate_role == ''){
        //   that.setData({
        //     num_a:16
        //   })
        // }else{
